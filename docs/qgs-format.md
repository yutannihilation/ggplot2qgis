...
Findings from `samples/nc_text.qgs` and `samples/nc_label.qgs` in the R
package repo (projects generated by the package, re-saved by QGIS 4.2
with labels enabled; the two differ only in the background's
`shapeDraw`). Labels need two things on the vector `<maplayer>`:

- the `labelsEnabled="1"` attribute, and
- a `<labeling type="simple">` element (QGIS writes it right after
  `<selection>`) whose single `<settings calloutType="simple">` child
  holds everything:
  - `<text-style fieldName="NAME" fontSize="10" fontSizeUnit="Point"
    textColor="..." ...>`: the labeled field (`isExpression="0"` for a
    plain field name) and the font. `fontSize` is in the unit named by
    `fontSizeUnit`; colors use the standard QGIS color serialization.
    Children, all written even when disabled: `<families/>`,
    `<text-buffer bufferDraw="0" .../>`, `<text-mask maskEnabled="0"
    .../>`, `<background>`, `<shadow shadowDraw="0" .../>`,
    `<dd_properties>` (an empty option collection) and
    `<substitutions/>`.
  - `<background shapeDraw="0|1" shapeType="0"
    shapeFillColor="...">`: the label background; `shapeType="0"` is a
    rectangle sized to the text. It embeds two symbols named
    `markerSymbol` (only used for marker-type backgrounds) and
    `fillSymbol` — QGIS draws a rectangle background with the
    fillSymbol, so its `color` option must mirror `shapeFillColor`.
    Their `<layer>` ids are empty strings, not UUIDs.
  - `<text-format>`: number formatting and multiline options (static
    defaults).
  - `<placement layerType="PointGeometry|LineGeometry|PolygonGeometry"
    placement="N" ...>`: where labels go. `placement` is
    QgsPalLayerSettings::Placement (0 = around point/centroid — what
    QGIS writes when enabling labels on a polygon layer — 1 = over
    point, 2 = along line).
  - `<rendering drawLabels="1" .../>`, another `<dd_properties>`, and a
    disabled `<callout type="simple">` whose `lineSymbol` option embeds
    a whole line `<symbol>` document as an escaped attribute string
    (with a symbol-layer UUID inside).

A labels-only layer (text without any drawn features) uses
`<renderer-v2 type="nullSymbol"/>` together with the labeling element.
